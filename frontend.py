...
class WordleApp:
    ...
    def show_stats(self):
        # перечитати актуальний stats.json через модуль stats
        stats_dict = stats.load_stats_dict()

        player_name = self.name_entry.get().strip()
        if not player_name:
            messagebox.showwarning("Statistics", "Please enter player name first.")
            return
        ...
        if not player_data:
            messagebox.showinfo(
                "Statistics",
                f"Статистику для цього користувача не знайдено: {player_name}"
            )
            return
    ...
    def submit_guess(self):
        ...
        if "FEEDBACK" in response or "WIN" in response:
            ...
            if "WIN" in response:
                ...
                messagebox.showinfo(
                    "Win",
                    f"{self.username}, you guessed the word: {guess.upper()}\n"
                    f"+{gain} score"
                )
            else:
